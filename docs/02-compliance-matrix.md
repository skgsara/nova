...
| §4.2.6 | Sync accuracy ±2×10⁻⁶, stability ±2×10⁻⁵; phasing automatic with manual adjustment | clock-rate from a median line-period fit over pairs of locked lines an eighth of the recording apart, segmented at every step in sync position; per-line dead-sector template lock with whole-line re-acquisition after a run of misses; manual phase/rate override built in the live renderer (session 21), wired to the window (session 23) and carried into the batch re-decode (session 24) | synthetic +100 ppm corrected (roundtrip [2]); −137 ppm recovered as −137.00 (roundtrip [8]); residual shear of the decoded image ≤10 ppm on all 20 library recordings, measured against the picture, not the metric (session 5); `fixture_warp` re-acquires across a stream time-skip (max_step 5.2 px). ISO §5.4.3 also makes manual phase adjustment a test item: `override_phase_seed` / `override_sync_fallback` (session 24) pin the re-decode, and `live_session` T12 pins that the operator's values reach it | synthetic+fixture ✓, manual adj. met end to end |
...
- WMO §5.2.3.2's two phasing waveforms (50/50 and 5/95) are covered
  ...
  decode so the two are always cross-checked.
  **Session 21 moves the manual half from "pending" to "built, not yet on
  screen."** `StreamPreview::set_phase_anchor` takes the adjustment in the
  form ISO asks an operator for — *where the dead sector is*, as a fraction
  of the line, never a delta — and applies it forward from the next row so
  that rows already drawn never move; `set_clock_ppm` is the rate half.
  Both are pinned by `live_preview`: the rows above an override are
  byte-identical to a render without it, exactly one row carries the mark,
  and the rows below move by the fraction asked for (−452 px against −453
  asked). The strongest evidence that this satisfies the *intent* of
  §4.2.6 rather than merely its wording is the unanchored case: on a
  white-only station with no phasing interval nothing in the transmission
  ...
  not behave the same way there, and the matrix should not be read as
  saying they do:** PHASE seeds the anchor search and is refined off it,
  while SYNC is used only where the period fit has no baseline — so on a
  healthy recording the operator's ppm is measured away from, in the
  direction of correct, and the PNG's QA header records which of the two
  happened. What remains is the *affordance*, not the capability: PHASE is
  typed as an image column rather than clicked on the picture [docs/05
  §8.3 item 1], and ISO §5.4.3's re-render-after-the-fact item needs the
  post-decode Apply/Auto lifecycle [docs/05 §8.5 items 2-4], which is
  still unbuilt.
